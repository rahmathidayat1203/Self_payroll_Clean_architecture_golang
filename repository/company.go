...
func (c *companyRepository) CreateOrUpdate(ctx context.Context, company *model.Company) (*model.Company, error) {
	...
	if err := c.Cfg.Database().WithContext(ctx).Model(companyModel).Save(&company).Find(companyModel).Error; err != nil {
		return nil, err
	}
	return company, nil
}
...
func (c *companyRepository) DebitBalance(ctx context.Context, amount int, note string) error {
	...
	amount = company.Balance - amount
	if err := c.Cfg.Database().WithContext(ctx).Model(company).Select("balance").Updates(map[string]interface{}{"balance": amount}).Error; err != nil {
		return err
	}
...
func (c *companyRepository) AddBalance(ctx context.Context, balance int) (*model.Company, error) {
	...
	balance = company.Balance + balance
	if err := c.Cfg.Database().WithContext(ctx).Model(company).Select("balance").Updates(map[string]interface{}{"balance": balance}).Error; err != nil {
		return nil, err
	}
	if err := c.Cfg.Database().WithContext(ctx).Model(company).Save(&company).Find(company).Last(&company).Error; err != nil {
		return nil, err
	}
	if err := c.Cfg.Database().WithContext(ctx).Create(&model.Transaction{
		Amount: balance,
