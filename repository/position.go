...
func (p *positionRepository) Delete(ctx context.Context, id int) error {
	...
	if err := p.Cfg.Database().WithContext(ctx).Where("id=?", id).Delete(&positions).Error; err != nil {
		return err
	}
	return nil
}
...
func (p *positionRepository) Fetch(ctx context.Context, limit, offset int) ([]*model.Position, error) {
	...
	var positions []*model.Position
	err := p.Cfg.Database().WithContext(ctx).Limit(limit).Offset(offset).Find(&positions).Error
	if err != nil {
